...
def plot_all(all_method_res_dict, max_time):
    config = {
        # "font.family": "serif",  # 使用衬线体
        "font.serif": ["serif"],  # 全局默认使用衬线宋体
        "font.size": 20,
    ...
    plot.xlabel("Testing Time (s)", ) #fontweight='bold')
    plot.ylabel("# Bugs", ) # fontweight='bold')
    plot.locator_params(axis='x', nbins=7)  # the number of num in axis.
    plot.locator_params(axis='y', nbins=12)  # the number of num in axis.
    ...
    for method, cumulative_bug_list in all_method_res_dict.items():
        ...
        plot.plot(bugs_id_list, cumulative_bug_list,   label=method, linewidth=3)
...
if __name__ == '__main__':
    ...
    for project in projects:
        ...
        plot_all(all_res_dict, max_time=max_time)
